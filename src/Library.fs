...
module Core =
        ...
        let defaultRootView head dataPage  =
            html [_lang "en" ; _style "height: 100vh;"] [
                ...
                body [ _style "height: 100%;" ] [
                    ...
                    script [ _src (defaultArg jsPath defaultJsPath); _type "module" ] []
                    //script [ _src "/js/vendors~main.js" ] []
                ]
            ]
    ...
    and InertiaResponse<'Props,'Shared, 'SSE> (
        ...
        member val private ReloadOnMount =
            {
                //shouldReload = false
                propsToEval = Eager // by default we get all values
                cacheStorage = StoreNone // don't store
                cacheRetrieval = SkipCache // don't check the cache
            }
            with get, set
            ...
            (?url:string, ?connectionId:string, ?version:string) : HttpHandler =
                fun next ctx ->
                    task {
                        ...
                        // Cast back to 'Props option type
                        ...
                        let reloadOnMount : ReloadOnMount =
                            let propsToEval =
                                ...
                                match isReload, isSSE, (isPartial || isFull) with
                                | true, false, _ -> // landing server-side as a reloaded page -- stop here
                                    Skip
                                | true, true, _ ->
                                    EagerOnly filter
                                // is not a reloaded page but is partial or full component ajax call
                                | false, _, true ->
                                    EagerOnly filter
                                | false, _, false -> // lazy (no data ajax) or full page -- use server defined
                                    x.ReloadOnMount.propsToEval
